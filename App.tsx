...
const App: React.FC = () => {
  ...
  const handleLogin = (userData: User) => {
    setUser(userData);
    // Explicitly navigate to root dashboard after login if router is used within App
  };
  ...
  const renderContent = () => {
    if (user.role === UserRole.CUSTOMER) {
      return (
        <CustomerDashboard 
          user={user} trips={trips} customers={customers} setTrips={setTrips} 
          drivers={drivers} onLogout={handleLogout} 
        />
      );
    }
    if (user.role === UserRole.DRIVER) {
      return (
        <DriverDashboard 
          user={user} trips={trips} setTrips={setTrips} drivers={drivers} 
          setDrivers={setDrivers} onLogout={handleLogout} 
        />
      );
    }
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-6 bg-black">
            <Routes>
              ...
              {/* Strict RBAC: Only Admin can access the /users route. Non-admins redirected to Dashboard with alert. */}
