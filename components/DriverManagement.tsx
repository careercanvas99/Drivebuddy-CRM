...
const DriverManagement: React.FC<DriverManagementProps> = ({ drivers, setDrivers, trips, users }) => {
  ...
  const handleAddDriver = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Database trigger fn_generate_driver_code handles the business ID.
      ...
      if (data) {
          const addedDriver: Driver = {
              id: data[0].id,
              displayId: data[0].driver_code, // Reading SQL generated code
              name: data[0].name,
              ...
              status: data[0].status as 'available' | 'busy' | 'inactive',
              location: [data[0].location_lat || 12.9716, data[0].location_lng || 77.5946]
          };
          setDrivers(prev => [...prev, addedDriver]);
          alert(`Driver Partner Registered: Business ID ${data[0].driver_code}`);
      }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Onboarding & business compliance</p>
        </div>
        <button 
            ...
            <form onSubmit={handleAddDriver} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input required className="w-full bg-gray-950 border border-gray-800 rounded-2xl p-4 text-sm focus:border-purple-500 outline-none shadow-inner" value={newDriver.name} onChange={e => setNewDriver({...newDriver, name: e.target.value})} placeholder="Enter name" />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-[10px] text-gray-500 uppercase px-3 font-black tracking-widest">License Number</label>
                  <input required className="w-full bg-gray-950 border border-gray-800 rounded-2xl p-4 text-sm focus:border-purple-500 outline-none shadow-inner font-mono" value={newDriver.licenseNumber} onChange={e => setNewDriver({...newDriver, licenseNumber: e.target.value})} placeholder="DLXXXXXXXXXXXXXXX" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] text-gray-500 uppercase px-3 font-black tracking-widest">Issue Date</label>
                  <input required type="date" className="w-full bg-gray-950 border border-gray-800 rounded-2xl p-4 text-sm focus:border-purple-500 outline-none shadow-inner" value={newDriver.issueDate} onChange={e => setNewDriver({...newDriver, issueDate: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] text-gray-500 uppercase px-3 font-black tracking-widest">Expiry Date</label>
                  <input required type="date" className="w-full bg-gray-950 border border-gray-800 rounded-2xl p-4 text-sm focus:border-purple-500 outline-none shadow-inner" value={newDriver.expiryDate} onChange={e => setNewDriver({...newDriver, expiryDate: e.target.value})} />
                </div>
              </div>
